...
public final class SetupActivity extends androidx.appcompat.app.AppCompatActivity {
    ...
    static boolean ready(Context c){return consent(c)&&ScanNotification.configured(c)&&ScanNotification.allowed(c);}
    ...
    private void render(){
        ...
        int next=!consent(this)?0:!ScanNotification.configured(this)?1:2;
        ...
        body.addView(GalleryUi.text(this,"시작하기 · "+(step+1)+" / 3",14));
        ...
        if(step==0){
            ...
            Button accept=GalleryUi.button(this,"동의하고 계속",()->{
                ...
                if(!getSharedPreferences("probe",0).contains("chromeDrawer"))edit.putBoolean("chromeDrawer",true);
            ...
            connection=GalleryUi.text(this,ScanNotification.connected()?"접근성: 허용됨 · 서비스 연결됨":"접근성: 허용됨 · 시스템 연결 대기",14);body.addView(connection);
            body.addView(GalleryUi.text(this,"검사·저장 상태와 중지 동작을 알림으로 제공합니다. 앱 알림과 ‘Chrome 검사 실행 상태’ 채널을 허용해 주세요. 상태바 아이콘 표시는 시스템 설정에 따라 다릅니다.",16));
    ...
    private void open(Intent intent){try{startActivity(intent);}catch(RuntimeException e){Toast.makeText(this,"설정 → 앱 → "+getString(R.string.app_name)+"에서 권한을 확인해 주세요.",Toast.LENGTH_LONG).show();}}
