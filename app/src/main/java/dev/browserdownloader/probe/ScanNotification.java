...
public final class ScanNotification extends BroadcastReceiver {
    ...
    private static String status = BuildConfig.SHARE_ONLY?"미디어 작업 대기":"Chrome 검사 대기";
    private static boolean mediaWorking;
    ...
    static void connection(Context context, boolean value) {
        connected = value; mediaWorking=false; status = BuildConfig.SHARE_ONLY?"미디어 작업 대기":"Chrome 검사 대기"; refresh(context);
    }
    static boolean configured(Context context) {
        if(BuildConfig.SHARE_ONLY)return false;
    ...
    static void status(Context context, String value) { status = value; if(BuildConfig.SHARE_ONLY)mediaWorking=GallerySession.get(context).busy; refresh(context); }
    static boolean allowed(Context context) {
        NotificationManager manager = context.getSystemService(NotificationManager.class);
        manager.createNotificationChannel(new NotificationChannel(CHANNEL, BuildConfig.SHARE_ONLY?"미디어 저장 상태":"Chrome 검사 실행 상태", NotificationManager.IMPORTANCE_LOW));
    ...
    static void refresh(Context context) {
        ...
        NotificationManager manager = context.getSystemService(NotificationManager.class);
        if ((BuildConfig.SHARE_ONLY?!mediaWorking:!connected) || !enabled(context) || hidden(context) || !allowed(context)) { manager.cancel(ID); return; }
        ...
        Notification notification = new Notification.Builder(app, CHANNEL)
                .setSmallIcon(R.drawable.ic_stat_mascot).setContentTitle(context.getApplicationInfo().loadLabel(context.getPackageManager())+(BuildConfig.SHARE_ONLY?" · 미디어 작업":" · 검사 활성"))
                ...
                .setTimeoutAfter(45_000).addAction(new Notification.Action.Builder(null, BuildConfig.SHARE_ONLY?"작업 중지":"검사·저장 중지", stop).build()).build();
    ...
    static void stop(Context context) {
        ...
        PreviewStore.get(context).cancelRequests(); PreviewStore.get(context).cancelSaves();
        mediaWorking=false; status = BuildConfig.SHARE_ONLY?"미디어 작업 대기":"Chrome 검사 대기"; refresh(context);
    }
